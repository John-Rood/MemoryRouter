...
> **Debug Mode:** Add `X-Debug: true` header or `?debug=true` to see memory/latency metadata in the response. Used by the dashboard playground.
...
MemoryRouter understands that not all memories are equal. Recent conversations matter more than old ones, but important facts should persist.

Our memory system automatically balances:
- **Immediate context** — What just happened in this conversation
- **Recent history** — Interactions from the past few days  
- **Long-term knowledge** — Important facts that should persist

You don't need to configure this — it just works. Your AI will remember what matters, when it matters.

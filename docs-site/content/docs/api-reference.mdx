...
|-----------|------|----------|-------------|
| `model` | string | Yes | Model identifier (e.g., `openai/gpt-5.1`, `anthropic/claude-sonnet-4.5`) |
...
```json
{
  ...
  "models": ["openai/gpt-5.1", "anthropic/claude-sonnet-4.5", "google/gemini-3-flash", "meta/llama-4-maverick", "..."],
...
Use the full model name with provider prefix (e.g., `openai/gpt-5.1`, `anthropic/claude-sonnet-4.5`).
...
```json
{
  ...
  "by_model": {
    ...
    "anthropic/claude-sonnet-4.5": {"requests": 347, "input_tokens": 54567}
  }
}
```

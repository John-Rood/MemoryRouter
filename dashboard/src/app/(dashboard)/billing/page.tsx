...
function CardBrandIcon({ brand }: { brand: string | null }) {
  ...
  if (brandLower === 'visa') {
    return (
      <div className={wrapperClass}>
        <FaCcVisa className={`${iconClass} text-[#1A1F71]`} />
      </div>
    );
  }
  ...
  if (brandLower === 'mastercard') {
    return (
      <div className={wrapperClass}>
        <FaCcMastercard className={`${iconClass} text-[#EB001B]`} />
      </div>
    );
  }
  ...
  if (brandLower === 'amex' || brandLower === 'american express') {
    return (
      <div className={wrapperClass}>
        <FaCcAmex className={`${iconClass} text-[#006FCF]`} />
      </div>
    );
  }
  ...
  if (brandLower === 'discover') {
    return (
      <div className={wrapperClass}>
        <FaCcDiscover className={`${iconClass} text-[#FF6000]`} />
      </div>
    );
  }
                ...
                <div key={tx.id} className="flex items-center justify-between py-3 border-b border-border/10 last:border-0">
                  ...
                  <div className="text-right">
                    <p className={`font-mono font-medium ${tx.type === 'credit' ? 'text-neon-green' : 'text-destructive'}`}>
                      {tx.type === 'credit' ? '+' : '-'}${Math.abs(tx.amountCents / 100).toFixed(
                        tx.type === 'credit' || Math.abs(tx.amountCents) >= 100 ? 2 : 4
                      )}
                    </p>
                  </div>
                </div>
